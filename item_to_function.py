...
@click.command()
@click.option(
    "-i", "--item-path", default='.', help="Path to item.yaml file or a directory containing one"
)
@click.option(
...
def item_to_function(
    ...
    # That means we need to search for items inside this direcotry
        ...
        for p in item_path.parent.glob('**/item.yaml'):
            inner_dir = p.parent
            try:
...
def create_function_yaml(
    ...
    function_object = code_to_function(
        ...
        with_doc=True,
        requirements_file=spec.get("requirements_file", ""),
    )
